...
function edit_room(id)
{
    ...
    xhr.onload = function()
    {
        ...
        edit_room_form.elements['name'].value =data.roomdata.name;
        edit_room_form.elements['decription'].value =data.roomdata.decription;
...
function submit_edit_room()
{
    ...
    data.append('decription', edit_room_form.elements['decription'].value);
...
function add_image()
{
    ...
    xhr.onload = function()
    {
        ...
        {
            alert('error', 'Wrong image format!', 'image-alert');
        }
        else if(this.responseText == 'inv_size'){
            alert('error', 'Image should be less than 2MB!', 'image-alert');
        }
        else if(this.responseText == 'upd_failed'){
            alert('error', 'Upload failed! Try Again.', 'image-alert');
        }
        else{
            alert('success', 'New image added!', 'image-alert');
            room_images(add_image_form.elements['room_id'].value, document.querySelector("#imageRoom .modal-title").innerText);
...
{
    document.querySelector("#imageRoom .modal-title").innerText =rname;
...
function del_image(img_id, room_id)
{
    ...
    xhr.onload = function()
    {
        if(this.responseText == 1)
        {
            ...
            alert('error', 'Unknow error!', 'image-alert');
        }
    }
    xhr.send(data);
}
...
function delete_room(room_id)
{
    if(confirm("Are you sure to remove this room?")){
        ...
        xhr.onload = function()
        {
            if(this.responseText == 1)
            {
                alert('success', 'The room has been removed!');
                get_all_rooms();
            }
            else{
                alert('error', 'Unknow error!');
